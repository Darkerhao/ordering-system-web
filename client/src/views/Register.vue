  ...
  <div class="auth-page">
    <div class="auth-card">
      <div class="auth-header">
        ...
        <p class="auth-subtitle">注册一个新账号，探索美味菜品</p>
      </div>

      <n-form ref="formRef" :model="form" :rules="rules" class="auth-form">
        <n-form-item path="username">
          <n-input v-model:value="form.username" placeholder="用户名（2-20个字符）" size="large">
            <template #prefix>👤</template>
          </n-input>
        </n-form-item>
        <n-form-item path="password">
          <n-input v-model:value="form.password" type="password" placeholder="密码（至少4个字符）" size="large" show-password-on="click">
            <template #prefix>🔒</template>
          </n-input>
        </n-form-item>
        <n-form-item path="confirmPassword">
          <n-input v-model:value="form.confirmPassword" type="password" placeholder="再次输入密码" size="large" show-password-on="click" @keyup.enter="handleRegister">
            <template #prefix>🔒</template>
          </n-input>
        </n-form-item>
        <n-button type="primary" block size="large" :loading="loading" class="submit-btn" @click="handleRegister">
          注 册
        </n-button>
      </n-form>
...
async function handleRegister() {
  ...
  loading.value = true
  try {
    await axios.post('/api/auth/register', { username: form.value.username, password: form.value.password })
